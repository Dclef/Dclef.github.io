...
function CalcLiuYaoPanPaiDuoHanZi()
{
    ...
    {
        alert( "请输入2到15个汉字用于起卦！" );
        return;
    }
    //检查字符串的汉字是否能查到笔画，查不到则无法排盘
    var arrBiHua = CheckHanZiBiHua(strDuoHanZi); //返回汉字笔画数的数组
    if( arrBiHua.length < 1 ) //数组是空的
    {
        alert( "存在非汉字或某个汉字笔画没查到，请尽量用常见汉字！" );
        return;
    }
    //alert( strDuoHanZi + "\n" + arrBiHua );
    ...
    //函数也填充起卦方式字符串
    //汉字起卦，第一个参数是原版汉字，第二个是笔画数记录数组
    //第三个参数表示时辰数目，如果时辰小于0就不用动爻加时辰
...
//第一个参数是字符串，第二个是笔画数组，第三个nShiChen如果为正数是动爻加时辰
 function GetLiuYaoBasicObjectDuoHanZiQG(strDuoHanZi , arrBiHua , nShiChen )
 {
    ...
    //计算下卦数理
    var nXiaSL = 0;
    for(var k=nLengthShang; k<nLength; k++)//剩下的就是下卦
    {
        nXiaSL += arrBiHua[k];
    }
    //总数理
    ...
    //设置摇卦方式字符串
    lyBasic.strQiGuaFangShi = "汉字：<b>" + strDuoHanZi 
