...
///////////////////////
 ...
 function InitSels()
 {
     for(var i=0; i<7; i++)
     {
         ...
         for(var j=0; j<49; j++)
         {
             addOp(curSel, j, Array49CardsNames[j]);
         }
 ...
 function OnCardSelectionChange(objSel, nPic)
 {
     ...
     if( ix < 10 )
     {
         strPicSrc = "longshen/0" + String(ix) + ".png" ;//凑两位数
     }
     else
     {
         strPicSrc = "longshen/" + String(ix) + ".png" ;
     }
     //设置图片源文件
...
function CalcLongShenPanPai()
{
    ...
    for(var i=0; i<7; i++)
    {
        ...
        if( arrJue[i] < 0 || arrJue[i] > 48 ) //合法范围0到48，共49张牌
        {
            alert("请选择正确的决名，并且七个不能重复，要按抽取顺序填！");
            return;
        };
        //检查重复
...
function GetLongShenPanPaiHTML(arrJue)//决有七个
{
    ...
    for(var i=0; i<7; i++)
    {
        strTemp =  PackCardName(String(i+1) + "、" + Array49CardsNames[ arrJue[i] ], i) 
...
//49个卡牌名
var Array49CardsNames = [
"太一", "太明", "太伤", "太劫", "太复", "太原", "太丁",
"太泽", "太于", "太相", "太专", "太兴", "太天", "太胜",
"太式", "太量", "太旺", "太正", "太幻", "太查", "太中",
"太传", "太冥", "太遂", "太文", "太随", "太问", "太更",
"太容", "太实", "太帝", "太新", "太间", "太语", "太令",
"太仓", "太归", "太开", "太阳", "太没", "太吾", "太行",
"太各", "太先", "太回", "太位", "太平", "太吉", "太灵"
];
